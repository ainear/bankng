...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [banks, products, categories, articles, bankers] = await Promise.all([
      ...
      prisma.banker.findMany({ where: { isActive: true }, select: { slug: true, updatedAt: true } }),
    ]);
    ...
    return [
      ...
      ...bankers.map((b) => ({ url: `${baseUrl}/banker/${b.slug}`, lastModified: b.updatedAt, changeFrequency: "weekly" as const, priority: 0.7 })),
    ];
  } catch {
